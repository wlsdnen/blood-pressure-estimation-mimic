    ...
    def __init__(self, signal_length, num_outputs, filter_sizes, num_filters):
        ...
        for i, filter_size in enumerate (filter_sizes):
            with tf.name_scope("conv-maxpool-%s" % filter_size):
                ...
                pooled2 = tf.reshape(pooled2, [-1, pooled2.get_shape()[2].value * num_filters])
        ...
        with tf.name_scope("fully_connected_layer1"):
            self.fclayer1 = tf.contrib.layers.fully_connected(self.h_drop, 4096, activation_fn=tf.nn.relu)
            self.h_drop1 = tf.nn.dropout(self.fclayer1, self.dropout_keep_prob)

        with tf.name_scope("fully_connected_layer2"):
            self.fclayer2 = tf.contrib.layers.fully_connected(self.h_drop1, 2048, activation_fn=tf.nn.relu)
            self.h_drop2 = tf.nn.dropout(self.fclayer2, self.dropout_keep_prob)

        with tf.name_scope("fully_connected_layer3"):
            self.fclayer3 = tf.contrib.layers.fully_connected(self.h_drop2, 1024, activation_fn=tf.nn.relu)
        ...
        #     self.fclayer6 = tf.contrib.layers.fully_connected(self.h_drop5, 256, activation_fn=tf.nn.relu)
        #     self.h_drop6 = tf.nn.dropout(self.fclayer6, self.dropout_keep_prob)
        #
        # with tf.name_scope("fully_connected_layer7"):
        #     self.fclayer7 = tf.contrib.layers.fully_connected(self.h_drop6, 128, activation_fn=tf.nn.relu)
        #     self.h_drop7 = tf.nn.dropout(self.fclayer7, self.dropout_keep_prob)
        #
        # with tf.name_scope("fully_connected_layer8"):
        #     self.fclayer8 = tf.contrib.layers.fully_connected(self.h_drop7, 64, activation_fn=tf.nn.relu)
        #     self.h_drop8 = tf.nn.dropout(self.fclayer8, self.dropout_keep_prob)

        with tf.name_scope("output_layer"):
            self.predictions = tf.contrib.layers.fully_connected(self.h_drop3, 1, activation_fn=None)
